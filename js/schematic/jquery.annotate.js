...
(function($) {
    ...
    $.fn.annotateImage = function(options) {
        ...
        this.canvas.height($('#dumpty-aspect-data').attr('data-height'));
        this.canvas.width($('#dumpty-aspect-data').attr('data-width'));
        this.canvas.css('background-image', 'url("' + this.attr('src') + '")');
        this.canvas.css('background-size', '100% 100%');
        this.canvas.children('.image-annotate-view, .image-annotate-edit').height(this.height());
        this.canvas.children('.image-annotate-view, .image-annotate-edit').width(this.width());

        /*$('.image-annotate-canvas').resizeWidthMaintainRatio($('.main-schemantic-image').width());
        $('.image-annotate-view').resizeWidthMaintainRatio($('.main-schemantic-image').width());
        $('.image-annotate-edit').resizeWidthMaintainRatio($('.main-schemantic-image').width());*/
    ...
    **/
    ...
    $.fn.annotateImage.createSaveButton = function(editable, image, note) {
        ...
        var ok = $('<button title="Send" type="submit" class="button send-button"><span><span>Send</span></span></button>');
    ...
    $.fn.annotateImage.createCancelButton = function(editable, image) {
        ...
        var cancel = $('<button class="image-annotate-edit-close button"><span><span>Cancel</span></span></button>');
    ...
    $.fn.annotateEdit = function(image, note) {
        ...
        this.area.css('height', $('.layer'+this.note.id).children().outerHeight() + 'px');
        this.area.css('width', $('.layer'+this.note.id).children().outerWidth() + 'px');
        this.area.css('left', $('.layer'+this.note.id).position().left + 'px');
        this.area.css('top',  $('.layer'+this.note.id).position().top + 'px');
        ...
        if(this.note.productcount == 0){
            var html = "";
            html += '<form><div class="title-note" id="prod_title">This item is available by request only. Please fill out the below form and we will contact you shortly.</div>';
            html += '<div id="request-form"><ul>';
                html += '<li><div class="no-display"><input id="schematic_url" type="hidden" name="schematic_url" value="'+document.URL+'"><input id="layer_id" type="hidden" name="layer_id" value="'+this.note.id+'"></div></li>';
                html += '<li><label>Name:</label><div class="input-box"><input id="name" name="name" type="text" value="'+$('#customer_name').val()+'"></div></li>';
                html += '<li><label>Email:</label><div class="input-box"><input id="email" name="email" type="text" value="'+$('#customer_email').val()+'"></div></li>';
                html += '<li><label>Phone:</label><div class="input-box"><input id="phone" name="phone" type="text" value="'+$('#customer_phone').val()+'"></div></li>';
                html += '<li><label>Comments:</label><div class="input-box"><textarea id="comments" name="comment"></textarea></div></li>';
            html += '</ul></div></form>';
            var form = $('<div id="image-annotate-edit-form">'+html+'</div>');

            $.ajax({
                url: image.deleteUrl,
                data: {layer_id:this.note.id},
                error: function(e) { alert("An error occured saving that note.") },
                success: function(data) {
            ...
            var j = 0;
            var i = 0;
            count = this.note.productcount;
            $.each(this.note.productdata, function (key, data) {
                if(i==0 || (i%4 == 0)){
                    html += '<div id="show-set'+j+'" class="product-group"';
                    if(j > 0){
                       html += ' style="display:none;"'; 
                    }
                    html += '>';
                }
                html += '<form onsubmit="return schematic_addtocart(this);" action="'+data.addtocart_url+'" method="post" class="group">';
                    html += '<div class="no-display"><input type="hidden" value="'+data.entity_id+'" name="product"></div>';
                    html += '<a class="product-image" title="'+data.name+'" href="'+data.url_path+'">';
                        html += '<img alt="'+data.name+'" src="'+data.thumbnail_image+'" />';
                    html += '</a>';
                    html += '<div class="product-info">';
                        html += '<a title="'+data.name+'" href="'+data.url_path+'" target="_blank">'+data.name+'</a>';
                        html += data.price_html;
                        html += data.addto_html;
                    html += '</div>';
                html += '</form>';
                i++;
                if((i!=0) && ((i%4 == 0) || (i == count))) { 
                    if(count>4){                   
                        if(i == count){
                            html += '<button onclick="show_set_product('+j+', 0)" class="button button-more"><span><span>More</span></span></button>';   
                        }
                        else{
                            html += '<button onclick="show_set_product('+j+', '+(j+1)+')" class="button button-more"><span><span>More</span></span></button>';
                        }
                    }
                    j++;
                    html += '</div>';
    ...
    $.fn.annotateView = function(image, note) {
        ...
        // Add the area
        this.area = $('<div class="image-annotate-area layer'+this.note.id  +' layer-edit-'+this.note.id  +  (this.editable ? ' image-annotate-area-editable' : '') + '"><div></div></div>');
        image.canvas.children('.image-annotate-view').prepend(this.area);

        // Add the note
        this.form = $('<div id="layer-note-'+this.note.id+'" class="image-annotate-note">' + note.text + '</div>');
        this.form.hide();
    ...
    $.fn.annotateView.prototype.show = function() {
        ...
        if (!this.editable) {
            ...
            /* My code to hover effect */ 
                $(".row-"+this.note.id).addClass('on-mouse');
            /* End of my code to hover effect */ 
        }
    };
    ...
    $.fn.annotateView.prototype.hide = function() {
        ...
        $(".row-"+this.note.id).removeClass('on-mouse');
    };
