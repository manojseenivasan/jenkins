...
class Ewall_Imagemarker_Adminhtml_ImagemarkerController extends Mage_Adminhtml_Controller_Action
{
	...
	public function indexAction() 
	{
	    ...
	    $this->_title($this->__("Manager Schematics"));
	    $this->_initAction();
	    $this->renderLayout();
	...
	{			    
	    ...
	    $block = $this->getLayout()->createBlock('Mage_Core_Block_Template', 'position', array('template' => 'imagemarker/position.phtml')); 
		...
		if ($post_data) {
			...
			{				
				...
				if(!empty($post_data['product_ids'])){
					$post_data['count'] = count(explode(',', $post_data['product_ids']));
				}
	...
	public function savePositionAction()
	{
		...
		if ($post_data) {
			try 
			{	
				foreach ($post_data['position'] as $id => $value) {
					$model = Mage::getModel("imagemarker/schematic")->load($id);
					$model->setPosition($value);
					$model->save();
				}		
				Mage::getSingleton("adminhtml/session")->addSuccess(Mage::helper("adminhtml")->__("Layer position was successfully saved"));
				Mage::getSingleton("adminhtml/session")->setImagemarkerData(false);
	...
	public function massResetAction()
	{
		try {
			...
			Mage::getSingleton("adminhtml/session")->addSuccess(Mage::helper("adminhtml")->__("Reseting schematics was successfully updated"));
		}
		catch (Exception $e) {
			Mage::getSingleton("adminhtml/session")->addError($e->getMessage());
		}
