...
class Ewall_Imagemarker_Block_Adminhtml_Schematic_Grid extends Mage_Adminhtml_Block_Widget_Grid
{
	...
	protected function _prepareColumns()
	{
		$this->addColumn("id", array(
			"header" => Mage::helper("imagemarker")->__("Layer Id"),
			"align" =>"left",
			"width" => "10%",
			"index" => "id",
			'sortable'  => false
		...
		$this->addColumn('position', array(
            'header'    => Mage::helper('imagemarker')->__('Position'),
            'width'     => '6%',
            'index'     => 'position',
            'sortable'  => false,
            'renderer'  => 'imagemarker/adminhtml_schematic_renderer_position'
        ));
	...
	{	
		$class = '';
		if($row->getId() == $this->getRequest()->getParam('layer')){
			$class .= 'active ';
		}
		$class .= 'layer-row row-'.$row->getId();
		return $class;
	}
}